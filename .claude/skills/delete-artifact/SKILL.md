...
description: Deletes an artifact definition, its compiled scripts and skills, given the artifact name, and removes dangling references to it from the codebase.
---
...
2. **deterministic** — Run `.gstudio/artifacts/artifact/scripts/delete/step-2.sh <name>`, where `<name>` is the artifact's name.

3. **deterministic** — Run `.gstudio/artifacts/artifact/scripts/delete/step-3.sh <name>` where `<name>` is the artifact name whose dangling references should be listed.

4. **llm** — Remove all the dangling references to the artifact, run the previous command to ensure all the references are gone. If not, fix it and run the command again.
