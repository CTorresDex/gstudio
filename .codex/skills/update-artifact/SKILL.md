...
description: Applies a natural-language change request to an existing artifact definition file, given the artifact name and the change request, exiting 1 if it does not exist.
---
...
1. **llm** — Input: <artifact-name> <change-request>

2. **deterministic** — Run .gstudio/artifacts/artifact/scripts/update/step-2.sh <name> from the project root, where <name> is the artifact's id.

3. **llm** — Apply the change request to the content of the artifact files as located by the rules.
