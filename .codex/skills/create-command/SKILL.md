...
1. **llm** — Input: command path and optionally the content of the command

2. **deterministic** — Run .gstudio/artifacts/command/scripts/create/step-2.sh "<command name>" ["<optional content>"], where `<command name>` is the space-separated command words (e.g. "compile artifact") and the optional second argument is the body content to place inside the exported function.
