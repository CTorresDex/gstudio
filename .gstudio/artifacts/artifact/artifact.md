...
<llm>Input: artifact name ({name}) and a description of the files the artifact governs: what they are, where they live and what every one of them must look like</llm>
<deterministic>
    Scaffolds the definition file from the template as defined at the rules, written without the indentation and with {name} substituted, exits 1 if the file already exists.
</deterministic>
<llm>
    Rewrite the scaffolded definition from the description: state the rules, then keep in every action only the steps that apply, wording each deterministic step so a script can settle it from its arguments alone and each llm step so an agent can judge it.
...
<llm>Input (Optional): search term</llm>
<deterministic>
    Lists all the artifacts defined in the codebase that contain the search term if provided, as located by the rules.
    Every folder holding a definition file is a single artifact.
...
<llm>Input: <artifact-name> <change-request></llm>
<deterministic>
    Exits 1 and prints error message if the definition file does not exist.
</deterministic>

## delete

<llm>Input: artifact name ({name})</llm>
<deterministic>
    Removes the artifact folder with its definition, scripts and manifest, and the skills compiled from it at every target, as defined at the rules; exits 1 if the definition file does not exist.
</deterministic>
<deterministic>
    Find all the existing references to the artifact in the codebase, that is every occurrence of its folder path (.gstudio/artifacts/{name}) or of one of its skill names ({action}-{name} for every action), and print them to stdout in the following format:

    [file path]:[line number]
</deterministic>
<llm>
    Remove all the dangling references to the artifact, run the previous command to ensure all the references are gone. If not, fix it and run the command again.
</llm>
...
<deterministic lang="ts">
    Evaluate, loading the definition with the Artifact class and computing every path and hash exactly the way the ArtifactCompiler class does, that:

    1. The definition file exists and its first line is `ID: {id}` with a non-empty id.
    2. The definition parses: the rules are not empty, every `## ` heading is one of the actions, none is duplicated, and every section body consists only of `<llm>` and `<deterministic>` steps.
    3. The first step of every action is an `<llm>` step.
    4. The artifact is compiled and up to date: the manifest exists, every deterministic step has a manifest entry with its current hash and an existing script at its path, every action has a manifest description with its current hash, and every skill exists at every target.

    If it complies with all the rules, exits 0.
    Otherwise, iterate over each discrepancy, print them to stdout and exit with 1, stating the compile command when a discrepancy is about compilation.
</deterministic>
<llm>
    Ensure that every deterministic step describes something a script can settle from its arguments alone and every llm step something that needs judgement, and that the rules are enough for the compiler to locate and shape every instance without guessing.
